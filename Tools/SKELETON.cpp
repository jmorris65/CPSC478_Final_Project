...
int Skeleton::readASFfile(const char* asf_filename, double scale)
{
  //open file
...
void Skeleton::compute_rotation_parent_child(Bone *parent, Bone *child)
{
  if(child != NULL)
  { 
    MATRIX4 Rz = RotationZ(-parent->axis_z * M_PI / 180.0);
    MATRIX4 Ry = RotationY(-parent->axis_y * M_PI / 180.0);
    MATRIX4 Rx = RotationX(-parent->axis_x * M_PI / 180.0);

    MATRIX4 tmp1 = Rx * Ry * Rz;
    
    Rz = RotationZ(child->axis_z * M_PI / 180.0);
    Ry = RotationY(child->axis_y * M_PI / 180.0);
    Rx = RotationX(child->axis_x * M_PI / 180.0);
    ...
    for (int x = 0; x < 4; x++)
      for (int y = 0; y < 4; y++)
        child->rot_parent_current[x][y] = tmp(y,x);
  }
}
...
void Skeleton::ComputeRotationToParentCoordSystem(Bone *bone)
{
  ...
  MATRIX4 Rz = RotationZ(bone[root].axis_z * M_PI / 180.0);
  MATRIX4 Ry = RotationY(bone[root].axis_y * M_PI / 180.0);
  MATRIX4 Rx = RotationX(bone[root].axis_x * M_PI / 180.0);

  MATRIX4 tmp2 = Rz * Ry * Rx;

  for (int x = 0; x < 4; x++)
    for (int y = 0; y < 4; y++)
      bone[root].rot_parent_current[x][y] = tmp2(y,x);
...
void Skeleton::RotateBoneDirToLocalCoordSystem()
{
  ...
  {
    VEC4 dir;
    dir[0] = m_pBoneList[i].dir[0];
    dir[1] = m_pBoneList[i].dir[1];
    dir[2] = m_pBoneList[i].dir[2];
    dir[3] = 1;

    MATRIX4 Rz = RotationZ(-m_pBoneList[i].axis_z * M_PI / 180.0);
    MATRIX4 Ry = RotationY(-m_pBoneList[i].axis_y * M_PI / 180.0);
    MATRIX4 Rx = RotationX(-m_pBoneList[i].axis_x * M_PI / 180.0);

    dir = Rx * Ry * Rz * dir;
    m_pBoneList[i].dir[0] = dir[0];
    m_pBoneList[i].dir[1] = dir[1];
    m_pBoneList[i].dir[2] = dir[2];
  }
}
...
// Constructor 
Skeleton::Skeleton(const char *asf_filename, double scale)
{
